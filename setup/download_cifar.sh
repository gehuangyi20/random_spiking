...
if [ -z "$1" ] ;
then
    output_dir=cifar_data/data
fi

echo $output_dir
dir_name=$(head /dev/urandom | tr -dc A-Za-z0-9 | head -c 13 ; echo '')
wget -c https://www.cs.toronto.edu/~kriz/cifar-10-binary.tar.gz -P "/tmp/$dir_name"
tar -xvzf "/tmp/${dir_name}/cifar-10-binary.tar.gz" -C "/tmp/$dir_name"

python3 setup/convert_cifar10_to_mnist_format.py "/tmp/${dir_name}/cifar-10-batches-bin/" $output_dir
